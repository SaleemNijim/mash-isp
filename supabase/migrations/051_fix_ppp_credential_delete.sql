-- ============================================================================
-- 051: إصلاح حذf PPP — تنظيف كل مراجع FK قبل الحذf
-- ============================================================================
-- المشكلة: ccu أو subscription_periods (حتى المُطلَقة/الم soft-deleted)
-- تبقى تشير إلى credential_id فتفشل DELETE برسالة FK.
-- الحل: فك/حذf كل المراجع بعد التحقق من الربط النشط (لغير المسؤول).

CREATE OR REPLACE FUNCTION unlink_credential_references(
...
BEGIN
  ...
  PERFORM unlink_credential_references(p_credential_id, v_tenant_id);
...
CREATE OR REPLACE FUNCTION bulk_hard_delete_internet_credentials(p_plan_id UUID)
RETURNS JSONB AS $$
DECLARE
...
REVOKE ALL ON FUNCTION unlink_credential_references(UUID, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION unlink_credential_references(UUID, UUID) TO authenticated;
