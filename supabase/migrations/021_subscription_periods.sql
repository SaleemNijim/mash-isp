-- ============================================================
-- MASH ISP — 021_subscription_periods.sql
-- سجل دورات اشتراك PPP (BB) لكل مشترك
-- ============================================================
...
CREATE TABLE subscription_periods (
  ...
  credential_id     UUID REFERENCES internet_credentials(id),
  username          TEXT,
  ...
  mac_address       TEXT,
  price             NUMERIC(10,2),
  billing_label     TEXT NOT NULL DEFAULT 'شهري',
  amount_due        NUMERIC(10,2),
  cash_amount       NUMERIC(10,2) NOT NULL DEFAULT 0,
  app_amount        NUMERIC(10,2) NOT NULL DEFAULT 0,
  discount_amount   NUMERIC(10,2) NOT NULL DEFAULT 0,
  balance_remaining NUMERIC(10,2) NOT NULL DEFAULT 0,
  paid_at           TIMESTAMPTZ,
  payment_id        UUID REFERENCES payments(id),
  pending_task_id   UUID REFERENCES pending_tasks(id),
  notes             TEXT,
  is_deleted        BOOLEAN NOT NULL DEFAULT false,
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_subscription_periods_tenant_customer
  ON subscription_periods (tenant_id, customer_id, period_start DESC)
  WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS idx_subscription_periods_subscription
  ON subscription_periods (tenant_id, subscription_id, period_start DESC)
  WHERE is_deleted = false;

ALTER TABLE subscription_periods ENABLE ROW LEVEL SECURITY;
ALTER TABLE subscription_periods FORCE ROW LEVEL SECURITY;

CREATE POLICY "subscription_periods_tenant_all" ON subscription_periods
  FOR ALL USING (tenant_id = get_tenant_id());

CREATE POLICY "subscription_periods_superadmin_all" ON subscription_periods
  FOR ALL USING (is_super_admin());

CREATE TRIGGER trg_soft_delete_subscription_periods
  AFTER UPDATE ON subscription_periods
  FOR EACH ROW EXECUTE FUNCTION log_soft_delete();

-- ── كريدنشالات BB المتاحة مع كلمة المرور ──
CREATE OR REPLACE FUNCTION list_available_bb_credentials()
RETURNS TABLE(id UUID, username TEXT, password TEXT) AS $$
BEGIN
...
REVOKE ALL ON FUNCTION list_available_bb_credentials() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION list_available_bb_credentials() TO authenticated;

-- ── إنشاء اشتراك PPP + أول دورة (بدون دفع) ──
...
BEGIN
  ...
  INSERT INTO subscription_periods (
    ...
    billing_label, amount_due, notes
  ) VALUES (
    ...
    'شهري', COALESCE(p_price, 0), p_notes
  );
...
REVOKE ALL ON FUNCTION create_subscription_with_period(UUID, TEXT, NUMERIC, DATE, DATE, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION create_subscription_with_period(UUID, TEXT, NUMERIC, DATE, DATE, TEXT, TEXT) TO authenticated;

-- ── دورة غير مدفوعة + مهمة معلقة (إشعار لاحقاً) ──
...
BEGIN
  ...
  INSERT INTO pending_tasks (
    ...
    COALESCE(p_amount_due, v_price, 0),
    v_due_at, 'pending'
  ...
  INSERT INTO subscription_periods (
    ...
    'شهري', COALESCE(p_amount_due, v_price, 0), v_task_id, p_notes
...
-- ── renew_subscription v2 — تجديد + سجل دورة ──
