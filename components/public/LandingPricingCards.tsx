  ...
  return (
    <div
      className={`relative rounded-xl p-6 flex flex-col gap-5 bg-mash-surface ${
        isPro ? 'border-2 border-primary-600' : 'border border-mash-border'
      }`}
    >
      {isPro && (
...
/** جلب الأسعار من المتصفح — يتجنب تحذيرات TLS على SSR لصفحة الهبوط */
export function LandingPricingCards() {
  ...
  if (!plans) {
    return (
      <div className="text-center text-sm text-mash-text-muted py-8" dir="rtl">
        جارِ تحميل الأسعار...
      </div>
    )
  }
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4" dir="rtl">
      {plans.map((plan) => {
        ...
        if (plan.billing_cycle === 'trial') {
          return <FreeTrialCard key={plan.slug} plan={plan} />
        }
        return <PlanCard key={plan.slug} plan={plan} />
      })}
    </div>
  )
}
